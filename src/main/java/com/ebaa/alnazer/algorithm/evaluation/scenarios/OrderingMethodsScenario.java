...
@Data
public class OrderingMethodsScenario extends BasicProblemEvaluationScenario {

    @Builder
    public OrderingMethodsScenario(int startSize, int endSize, int step, boolean hasShortcuts, PlanningAlgorithm algorithm, File outputDirectory) {
        ...
        try {
            ...
            try {
                for (int currentSize = startSize; currentSize <= endSize; currentSize += step) {
                    ...
                    ProblemFileManager.generateOrderingMethodsMoveContainerProblemFile(currentSize, hasShortcuts, problemFilePath);
    ...
    public String toString() {
        return String.format("orderingMethods-start%s-end%s-step%s-shortcut%s-algo_%s_", startSize, endSize, step,hasShortcuts, algorithm.toString());
    }
}
